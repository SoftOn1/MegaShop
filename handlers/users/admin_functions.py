...
class getph(StatesGroup):
    photo = State()
    phtext = State()

@dp.message_handler(state=getph.phtext)
async def konkname(message: types.Message, state: FSMContext):
    phtext = message.text
    await state.update_data(phtext=phtext)
    await bot.send_message(message.chat.id,text = '🖼 Пришли картинку для расслыки')
    await getph.photo.set()
    

@dp.message_handler(state=getph.photo,content_types=['photo'])
async def konkname(message: types.Message, state: FSMContext):
    ...
    for user in users:
        try:
            ...
            await bot.send_photo(user[1], photo = open(f'{name}.jpg', 'rb'),caption = tx,parse_mode = 'Markdown', reply_markup=clear_address)
            receive_users += 1
    ...
    await bot.send_message(message.chat.id,
                           ...
                           f"<b>👤 Получили сообщение:</b> <code>{receive_users} ✅</code>\n"
                           f"<b>👤 Заблокировали бота:</b> <code>{block_users} ❌</code>",reply_markup=get_functions_func(message.from_user.id))
    await state.finish()
...
async def send_ad_all_users(message: types.Message, state: FSMContext):
    m_types = types.ReplyKeyboardMarkup(resize_keyboard = True)
    text = types.KeyboardButton(text = '📜 Текст')
    photo = types.KeyboardButton(text = '🖼 Картинка')
    ph_txt = types.KeyboardButton(text = '🖼 Текст + картинка')
    back =  types.KeyboardButton(text="⬅ На главную")
    m_types.add(text,ph_txt)
    m_types.add(back)
    await bot.send_message(message.chat.id,text = '<b>Выберите тип рассылки</b>',reply_markup = m_types)


@dp.message_handler(IsAdmin(), text="🖼 Текст + картинка", state="*")
async def send_ad_all_users(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer("🖍 <b>Введите текст для рассылки:</b>")
    await getph.phtext.set()

@dp.message_handler(IsAdmin(), text="📜 Текст", state="*")
async def send_ad_all_users(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer("🖍 <b>Введите текст для рассылки:</b>")
...
@dp.message_handler(IsAdmin(), text="⬅ На главную", state="*")
...
async def send_message_to_user(message, user_id):
    ...
    for user in users:
        try:
            clear_address = InlineKeyboardMarkup()
            clear_address.add(InlineKeyboardButton(text="❌Закрыть", callback_data="close"))
            await bot.send_message(user[1], message,parse_mode='Markdown', reply_markup=clear_address)
            receive_users += 1
        except:
            block_users += 1
        await asyncio.sleep(0.05)
    await bot.send_message(user_id,
                           f"<b>📢 Рассылка была завершена ☑</b>\n"
                           f"<b>👤 Получили сообщение:</b> <code>{receive_users} ✅</code>\n"
                           f"<b>👤 Заблокировали бота:</b> <code>{block_users} ❌</code>")
    ...
    if message.text.startswith("+"):
        ...
        if get_purchase is not None:
            ...
            await message.answer(f"<b>📃 Чек:</b> <code>#{get_purchase[4]}</code>\n"
                                 ...
                                 f"📦 Куплено товаров: <code>{get_purchase[5]}/code>\n"
